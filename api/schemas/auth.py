...
class UserPublic(CamelModel):
    id: str = Field(description="Stable user id, e.g. `user_...`.")
...
class Tokens(CamelModel):
    access_token: str = Field(description="Short-lived HMAC-signed access token (Bearer).")
...
class SignupRequest(CamelModel):
    ...
    password: str = Field(
        description="Password (dev stub: not verified).", examples=["password123"]
    )
...
class LoginRequest(CamelModel):
    ...
    password: str = Field(
        description="Password (dev stub: any value accepted).", examples=["password123"]
    )
...
class LogoutResponse(CamelModel):
    success: bool = Field(default=True, description="Always true; logout is best-effort.")
